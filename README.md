...
cd vertex_app
flutter pub get
flutter run --dart-define=API_BASE_URL=https://vertex-ids-backend.onrender.com
```
...
- The mobile app defaults to the production Render backend.
- Override with `--dart-define=API_BASE_URL=...` only if you need a different environment.
